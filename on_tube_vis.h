...
protected:
	...
	{		
		if (is_2D())
		{
			glyph_dimension = GlyphDimension::GD_3D;
	...
	void toggle_glyph_method()
	{
		if (render.style3D.is_tube_surface_pipeline())
			render.style3D.glyph_method = glyph3D_spline_tube_render_style::GM_BILLBOARD_PIPELINE;
		...
		on_set(&render.style3D.glyph_method);
	}
	std::string get_glyph_method_toggle_label()
	{
		std::string label = "GlyphMethod: ";
		label += render.style3D.is_tube_surface_pipeline() ? "TubeSurface" : "Billboard";
		label += " (toggle)";
		return label;
	}
	...
	struct {
		...
		//TODO THESIS
		glyph3D_spline_tube_render_style style3D;
	...
	struct {
		...
		// THESIS
		struct {
			GlyphDimension current_glyph_dimension = GlyphDimension::GD_2D;
			bool was_toggled = false;
			bool check_toggled(void) { const bool toggled = was_toggled; was_toggled = false; return toggled; }
			cgv::gui::button_ptr button;
		}dim_toggle;
	...
	//THESIS2: 
	shader_define_map build_glyphs3D_tube_shading_defines(bool isFront);

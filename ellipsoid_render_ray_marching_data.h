...
#include <cgv_gl/render_data_base.h>

namespace cgv {
	namespace render {

		/// @brief Render data for ellipsoid geometry with support for the ellipsoid_renderer_ray_marching. See render_data_base.
		...
		class ellipsoid_render_ray_marching_data : public render_data_base<ellipsoid_renderer_ray_marching, ellipsoid_render_ray_marching_style, ColorType> {
		private:
			// Base class we're going to use virtual functions from
			typedef render_data_base<ellipsoid_renderer_ray_marching, ellipsoid_render_ray_marching_style, ColorType> super;

			ellipsoid_renderer_ray_marching& ref_renderer_singleton(context& ctx, int ref_count_change = 0) override {
				return ref_ellipsoid_renderer_ray_marching(ctx, ref_count_change);
			}

		protected:
			/// @brief See render_data_base::transfer.
			bool transfer(context& ctx, ellipsoid_renderer_ray_marching& r) override {

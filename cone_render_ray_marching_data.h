...
namespace cgv {
	namespace render {
		...
		class cone_render_ray_marching_data : public render_data_base<cone_renderer_ray_marching, cone_render_ray_marching_style, ColorType> {
		private:
			// Base class we're going to use virtual functions from
			typedef render_data_base<cone_renderer_ray_marching, cone_render_ray_marching_style, ColorType> super;

			cone_renderer_ray_marching& ref_renderer_singleton(context& ctx, int ref_count_change = 0) override {
				return ref_cone_renderer_ray_marching(ctx, ref_count_change);
			}

		protected:
			/// @brief See render_data_base::transfer.
			bool transfer(context& ctx, cone_renderer_ray_marching& r) override {

...
namespace cgv {
	namespace render {
		...
		glyph3D_render_style::glyph3D_render_style() : material("default")
		{
		...
		void glyph3D_renderer::enable_attribute_array_manager(const context& ctx, attribute_array_manager& aam)
		{
		...
		void glyph3D_renderer::disable_attribute_array_manager(const context& ctx, attribute_array_manager& aam)
		{
			group_renderer::disable_attribute_array_manager(ctx, aam);
			has_normals = false;
			has_texcoords = false;
		}
		void glyph3D_renderer::remove_normal_array(const context& ctx) {
			has_normals = false;
			remove_attribute_array(ctx, "normal");
		}
		void glyph3D_renderer::remove_texcoord_array(const context& ctx) {
			has_texcoords = false;
		...
		void glyph3D_renderer::set_normal_array(const context& ctx, type_descriptor element_type, const vertex_buffer& vbo, size_t offset_in_bytes, size_t nr_elements, unsigned stride_in_bytes)
		{
			has_normals = true;
		...
		void glyph3D_renderer::set_texcoord_array(const context& ctx, type_descriptor element_type, const vertex_buffer& vbo, size_t offset_in_bytes, size_t nr_elements, unsigned stride_in_bytes)
		{
			has_texcoords = true;
			set_attribute_array(ctx, "texcoord", element_type, vbo, offset_in_bytes, nr_elements, stride_in_bytes);

		}
		bool glyph3D_renderer::enable(context& ctx)
		{
			bool res = group_renderer::enable(ctx);
			const glyph3D_render_style& srs = get_style<glyph3D_render_style>();
		...
		bool glyph3D_renderer::disable(context& ctx)
		{
			const glyph3D_render_style& srs = get_style<glyph3D_render_style>();
			if (cull_per_primitive) {
				if (srs.culling_mode != CM_OFF)
		...
		bool glyph3D_render_style_reflect::self_reflect(cgv::reflect::reflection_handler& rh)
		{
			return
		...
		cgv::reflect::extern_reflection_traits<glyph3D_render_style, glyph3D_render_style_reflect> get_reflection_traits(const glyph3D_render_style&)
		{
			return cgv::reflect::extern_reflection_traits<glyph3D_render_style, glyph3D_render_style_reflect>();
		}
	}
}
...
namespace cgv {
	namespace gui {
		...
		struct glyph3D_render_style_gui_creator : public gui_creator
		{
			...
			{
				if (value_type != cgv::type::info::type_name<cgv::render::glyph3D_render_style>::get_name())
					return false;
				cgv::render::glyph3D_render_style* srs_ptr = reinterpret_cast<cgv::render::glyph3D_render_style*>(value_ptr);

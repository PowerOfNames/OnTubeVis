...
void on_tube_vis::handle_member_change(const cgv::utils::pointer_test& m) {
	...
	if(data_set_changed) {
		...
		else
		{
			...
			shaders.reload(ctx, "tube_shading_back", build_glyphs3D_tube_shading_defines(false));
			shaders.reload(ctx, "tube_shading_front", build_glyphs3D_tube_shading_defines(true));
		}
	...
	if (m.is(glyph_dimension))
	{
		...
		//THESIS2: quick solution. -> here to update grid mode (no grid on backside, but on front
		shaders.reload(ctx, "tube_shading_back", build_glyphs3D_tube_shading_defines(false));
		shaders.reload(ctx, "tube_shading_front", build_glyphs3D_tube_shading_defines(true));
	}
...
{	
	...
	const auto find_buffer_idx = [this](const std::unordered_map<uint32_t, std::pair<uint32_t, uint32_t>>& idx_to_idx, const std::string& attrib_name, const glyph_shape* glyph, uint32_t base_idx) {
		...
		//Set attrib_idx
		return found ? std::pair<uint32_t, uint32_t>(base_idx + idx_to_idx.at(glyph_attrib_idx).first, idx_to_idx.at(glyph_attrib_idx).second ) : std::pair<uint32_t, uint32_t>(0, 0); //return 0.0 -> s | return 1 -> debug_int -> choose 0 -> potential check and set to default value if idx == 0
		};
	...
	const auto& mapping_vec_mag = mapping[vec_mag_idcs.second];

	const auto orientation_w_idcs = find_buffer_idx(glyph_attrib_idx_to_buffer_idx, "ori_w", layer_config.shape_ptr, last_nonmapped_attrib_idx);
	const uint32_t orientation_w_idx = orientation_w_idcs.first;
	//const auto& mapping_ori_w = mapping[orientation_w_idcs.second];

	const auto orientation_i_idcs = find_buffer_idx(glyph_attrib_idx_to_buffer_idx, "ori_i", layer_config.shape_ptr, last_nonmapped_attrib_idx);
	const uint32_t orientation_i_idx = orientation_i_idcs.first;
	//const auto& mapping_ori_i = mapping[orientation_i_idcs.second];

	const auto orientation_j_idcs = find_buffer_idx(glyph_attrib_idx_to_buffer_idx, "ori_j", layer_config.shape_ptr, last_nonmapped_attrib_idx);
	const uint32_t orientation_j_idx = orientation_j_idcs.first;
	//const auto& mapping_ori_j = mapping[orientation_j_idcs.second];
				...
				switch (glyph_type)
				{
					...
					case GT_3D_ELLIPSOID_3x3_TENSOR:
					{
						...
						ellipsoids.add_orientation({
							...
							attribs.data[attrib_base_idx + orientation_i_idx],
					...
					case GT_3D_CONE_VECTOR:
					{
						...
						const vec3 v = vec3(
							attribs.data[attrib_base_idx + vec_x_idx],
							attribs.data[attrib_base_idx + vec_y_idx],
							attribs.data[attrib_base_idx + vec_z_idx]
						);
	...
	if(traj_mgr.has_data())
	{
		...
		case DRM_NONE:
			...
			if (render.style3D.is_tube_surface_pipeline())
				...
				draw_trajectories_glyphs3D(ctx);
			break;
		case DRM_NODES:
...
//THESIS2:
void on_tube_vis::draw_tube_geometry(context& ctx, bool sortNodes /*= true*/)
{
	// - view-related info
...
void on_tube_vis::draw_tube_geometry_glyph3D(context& ctx, bool sortNodes /*= true*/)
{
	// - view-related info
	const vec3& cyclopic_eye = view_ptr->get_eye();
	const vec3& view_dir = view_ptr->get_view_dir();
...
void on_tube_vis::draw_trajectories(context& ctx)
{
	// common init
	if (is_2D())
		draw_tube_geometry(ctx, true);
	else
	{
		render.style3D.tube_backside_render.clip_caps = false;
		draw_tube_geometry_glyph3D(ctx, true);
	}
...
void on_tube_vis::draw_trajectories_glyphs3D(context& ctx)
{
	...
	draw_glyphs3D(ctx);
	render.style3D.cull_frontface = false;
...
//   (workaround for longstanding NVIDIA driver bug preventing GPU-internal PBO transfers to GL_DEPTH_COMPONENT formats)
	...
	{
		// perform the deferred shading pass and draw the image into the shading framebuffer when not using OptiX (for now)
		shader_program& prog = shaders.get("tube_shading_back");
...
{
	draw_tube_geometry_glyph3D(ctx, false);
	...
	{
		// perform the deferred shading pass and draw the image into the shading framebuffer when not using OptiX (for now)
		shader_program& prog = shaders.get("tube_shading_front");
...
shader_define_map on_tube_vis::build_glyphs3D_tube_shading_defines(bool isFront) {
	...
	shader_code::set_define(defines, "GRID_MODE", grid_mode, isFront ? GM_COLOR : GM_NONE);
	unsigned gs = static_cast<unsigned>(grid_normal_settings);
	if (grid_normal_inwards) gs += 4u;
	if (grid_normal_variant) gs += 8u;
	shader_code::set_define(defines, "GRID_NORMAL_SETTINGS", gs, 0u);
